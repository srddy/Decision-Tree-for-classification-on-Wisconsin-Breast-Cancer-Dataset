...
def majority_label(rows) :
    ...
    for row in rows :
        label = row[-1]
        if label not in counts1:
            counts1[label] = 0
        counts1[label] += 1
    maxItem = max(counts1.items(), key=lambda x : x[1]);
    return maxItem[0];
...
## TODO: Step 3
def entropy(rows):
    row_set = class_counts(rows)
    req_entropy = 0
    for key,value in row_set.items():
        req_entropy = req_entropy + ((value/len(rows)) * (math.log(value/len(rows)))/math.log(2))*(-1);
...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        ...
        self.predictedClassLabel = majority_label(rows);
        self.rows = rows;
        self.id = id
        self.depth = depth;
...
def print_tree(node, spacing=""):
    ...
    print(spacing + 'Depth of the node  :', node.depth)
    print(spacing + 'ID of the node  :' , node.id);
...
def computeAccuracy(rows, node):
    ...
    for row in rows: 
        label = row[-1]
        predicted_class_variable = classify(row, node)
        if label == predicted_class_variable :
            numAccurate = numAccurate + 1

    return round(numAccurate/total_rows, 2)
